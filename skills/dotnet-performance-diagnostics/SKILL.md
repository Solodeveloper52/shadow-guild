---
name: dotnet-performance-diagnostics
description: Plan structured decisions and execution paths for software delivery and system operation in the dotnet-performance context.
rank: a
class: backend
domain: software-engineering
subdomain: dotnet-performance
maturity: pro
---

# Dotnet Performance Diagnostics

Use this skill to shape ambiguous work into a staged plan with explicit tradeoffs for software delivery and system operation. The method assumes you need evidence-backed decisions, reusable artifacts, and a review trail that survives beyond the current session.
...
- Another skill already governs the narrower problem more directly than dotnet performance diagnostics.

## Trigger Question

What evidence, constraints, and decision criteria do we need before we can confidently build a decision-ready plan for software delivery and system operation?

## Required Inputs

- Objective statement for dotnet performance diagnostics with scope and non-goals.
...
- Dotnet Performance Diagnostics working document with assumptions, options, recommendation, and review notes.
...
- Use short review loops; long silent drafting phases tend to hide assumption drift in dotnet-performance.
- When handing off, package the recommendation, evidence links, and next decision checkpoint together.

## Companion Assets

- templates/dotnet-performance-investigation.md
- examples/api-latency-regression-example.md
- references/dotnet-trace-counter-checklist.md
