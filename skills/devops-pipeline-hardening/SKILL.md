---
name: devops-pipeline-hardening
description: Plan structured decisions and execution paths for security posture, delivery controls, and operational resilience in the pipeline-hardening context.
rank: a
class: devops
domain: security-devops
subdomain: pipeline-hardening
maturity: pro
---

# Devops Pipeline Hardening

Use this skill to shape ambiguous work into a staged plan with explicit tradeoffs for security posture, delivery controls, and operational resilience. The method assumes you need evidence-backed decisions, reusable artifacts, and a review trail that survives beyond the current session.
...
- Another skill already governs the narrower problem more directly than devops pipeline hardening.

## Trigger Question

What evidence, constraints, and decision criteria do we need before we can confidently build a decision-ready plan for security posture, delivery controls, and operational resilience?

## Required Inputs

- Objective statement for devops pipeline hardening with scope and non-goals.
...
- Devops Pipeline Hardening working document with assumptions, options, recommendation, and review notes.
...
- Use short review loops; long silent drafting phases tend to hide assumption drift in pipeline-hardening.
- When handing off, package the recommendation, evidence links, and next decision checkpoint together.

## Companion Assets

- templates/pipeline-hardening-checklist.md
- examples/github-actions-hardening-example.md
- references/release-gate-control-points.md
