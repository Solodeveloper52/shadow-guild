...
description: Define enforceable quality gates for language-specific implementation quality and maintainability in the python context.
rank: a
class: python
...
Use this skill to define pass fail controls that catch bad changes before they escape for language-specific implementation quality and maintainability. The method assumes you need evidence-backed decisions, reusable artifacts, and a review trail that survives beyond the current session.

## Use This Skill When

- You are preparing to set and enforce decision gates and need a repeatable operating method instead of ad hoc judgment.
- The work affects developers, reviewers, and maintainers and the decision needs explicit tradeoffs, evidence, and ownership.
...
- Another skill already governs the narrower problem more directly than python quality gate.

## Trigger Question

What evidence, constraints, and decision criteria do we need before we can confidently set and enforce decision gates for language-specific implementation quality and maintainability?

## Required Inputs

- Objective statement for python quality gate with scope and non-goals.
- Current state evidence covering compiler output, lint results, test evidence, profiling data.
- Named stakeholders: developers, reviewers, and maintainers.
...
- Python Quality Gate working document with assumptions, options, recommendation, and review notes.
...
Objective: Clarify which failures python quality gate must prevent.
...
- Prefer options that reduce ambiguity for developers, reviewers, and maintainers without hiding material tradeoffs.
...
- The output references concrete evidence from compiler output, lint results, test evidence, profiling data rather than generic advice.
...
- The team optimizes for speed and skips the tradeoff analysis needed by developers, reviewers, and maintainers.
...
- Quality cycle time from request to approved artifact.
...
- Pull in developers, reviewers, and maintainers early enough to validate constraints before solution shaping begins.
- Keep open questions separate from blocked items so reviewers can distinguish uncertainty from missing work.
- Use short review loops; long silent drafting phases tend to hide assumption drift in python.
- When handing off, package the recommendation, evidence links, and next decision checkpoint together.

## Companion Assets

- templates/python-quality-gate-template.md
- examples/fintech-quality-gates-case-study.md
- references/python-quality-gate-pitfalls.md
