---
name: test-maintainability-audit
description: Run evidence-backed audits and governance reviews for software delivery and system operation in the test-maintainability context.
rank: a
class: software
domain: software-engineering
subdomain: test-maintainability
maturity: pro
---

# Test Maintainability Audit

Use this skill to inspect the current state, surface real risk, and drive remediation with ownership for software delivery and system operation. The method assumes you need evidence-backed decisions, reusable artifacts, and a review trail that survives beyond the current session.
...
- Another skill already governs the narrower problem more directly than test maintainability audit.

## Trigger Question

What evidence, constraints, and decision criteria do we need before we can confidently audit the current state for software delivery and system operation?

## Required Inputs

- Objective statement for test maintainability audit with scope and non-goals.
...
- Test Maintainability Audit working document with assumptions, options, recommendation, and review notes.
...
- Use short review loops; long silent drafting phases tend to hide assumption drift in test-maintainability.
- When handing off, package the recommendation, evidence links, and next decision checkpoint together.

## Companion Assets

- templates/test-maintainability-framework.md
- examples/test-audit-case-study.md
- references/test-audit-pitfalls.md
